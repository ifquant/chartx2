...
- `LegendViewModel`
  ...
  - `Line Break` lower panes now run through the chart engine's own chart-context secondary path instead of a workbench-only synthetic data fork, and API coverage now also locks requested-context moving-average merge on top of that compressed main sequence
