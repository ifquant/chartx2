...
- `LegendViewModel`
  ...
  - `Renko`, `Kagi`, and `Point Figure` now all rely on the engine's own chart-context secondary path in the workbench as well; `Point Figure` no longer needs a workbench-only secondary-pane suppression branch, and requested-context compare merge is now also locked against the compressed `Point Figure` main path
