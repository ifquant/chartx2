...
- chart-owned drawings now have a first independent registry and snapshot path through minimal `horizontal-line` and `trend-line` objects, and they now also support first-click selection/hit-testing, minimal keyboard selection commands (`Escape` to clear, `Delete/Backspace` to remove), the first `trend-line` endpoint drag path from a selected line body, and a first hover affordance that promotes the cursor to `move` over editable selected trend-line handles; drawings are still far from a full entity system with grouping, richer edit handles, and z-order control
- `LegendViewModel`
