...
- chart-owned drawings now have a first independent registry and snapshot path through minimal `horizontal-line` and `trend-line` objects, and they now also support first-click selection/hit-testing plus minimal keyboard selection commands (`Escape` to clear, `Delete/Backspace` to remove); drawings are still far from a full entity system with grouping, drag editing, and z-order control
- `LegendViewModel`
