...
- a first pane lifecycle surface now exists
  ...
  - `subscribePaneEvents() / unsubscribePaneEvents()` on the chart API, with a pane-state snapshot
...
- pane handles now expose a small options surface, a resize callback, and a chart-level pane event bus with pane snapshots, but pane-local APIs are still much narrower than lightweight-charts
