...
- `MergeEngine`
  - the first model-layer `StudyMergeEngine` now exists and owns the requested-context/chart-context alignment path for `carry-forward`, `exact`, and the current `gaps` placeholder behavior
  - the harness now routes chart-context secondary series, requested-context compare, and requested-context moving-average inputs through this engine instead of directly calling a loose merge helper
  - this is still only the merge boundary: `gaps` currently behaves like `exact`, and a richer source-context/request registry is still not implemented
- `DrawingSource`
- markers and price lines exist as early annotation paths
- chart-owned drawings now have a first independent registry and snapshot path through minimal `horizontal-line` and `trend-line` objects, and they now also support first-click selection/hit-testing, minimal keyboard selection commands (`Escape` to clear, `Delete/Backspace` to remove), the first `trend-line` endpoint drag path from a selected line body, a first hover affordance that promotes the cursor to `move` over editable selected trend-line handles, minimal price snapping with OHLC source filtering, visible snap guides, chart-level magnet controls, an independent time-magnet enable toggle, configurable price/time snap tolerances, a first explicit time-magnet policy surface (`nearest` / `previous` / `next`), a first shared drawing property system for tool-level magnet overrides across `trend-line` and `horizontal-line`, the first selection-driven drawing property API for inspector-style read/update flows, a first drawing property schema surface that exposes inspector sections/field metadata for `appearance`, `geometry`, and `magnet`, and now the workbench demo consumes that schema through a minimal drawing inspector UI with first-pass field metadata for `min` / `step` / `required` / `select options`, inline validation, the first trend-line cross-field validation for endpoint ordering/non-overlap, matching runtime validation in the chart API itself, a first model-layer drawing validator registry that both `horizontal-line` and `trend-line` route through, and pre-restore drawing validation so invalid snapshots are rejected before current drawings are mutated; drawings are still far from a full entity system with grouping, richer edit handles, richer time-source policy, undo/redo, and z-order control
- `LegendViewModel`
  ...
  - `Line Break` lower panes now run through the chart engine's own chart-context secondary path instead of a workbench-only synthetic data fork, and requested-context moving-average on top of that compressed main sequence now also has a deterministic parity contract instead of only route/pane coverage
  - `Renko`, `Kagi`, and `Point Figure` now all rely on the engine's own chart-context secondary path in the workbench as well; `Point Figure` no longer needs a workbench-only secondary-pane suppression branch, requested-context compare / moving-average merge now has an explicit non-time API contract matrix across `Point Figure`, `Line Break`, `Renko`, and `Kagi`, and both `Renko + requested-context moving-average` and `Kagi + requested-context moving-average` now have deterministic parity contracts that lock the merged SMA values themselves instead of only route/pane metadata
  - `Kagi` no longer reuses the generic `segment` renderer in the workbench; it now has a dedicated renderer, a more conservative auto-reversal heuristic that targets a readable swing count on long raw OHLC inputs, and workbench lower panes that derive from the current synthetic `Kagi` rows instead of stretching the chart back across raw time bars
  - `Kagi` now also has its own option surface instead of living on hard-coded defaults: the engine tracks `auto / fixed / atr / percentage` reversal modes, snapshot/restore carries that state, and the workbench exposes mode switches plus scale / ATR length / percentage / fixed reversal controls
  - `Columns` and `HLC Area` are now real main chart types instead of renderer-only placeholders: both are routed through the chart-type registry, `Columns` uses raw OHLCV input with a dedicated columns chart type instead of only the older histogram path, and `HLC Area` now renders a filled area from the synthetic HLC average rather than staying as an empty renderer slot
  - the workbench's default `horizontal-line` and `trend-line` drawings now derive their anchors from the current chart type's effective main rows, so `Line Break`, `Renko`, `Kagi`, and `Point Figure` no longer have to fall back to raw-candlestick demo anchors just to show default drawings
...
- chart snapshots/templates now also persist first chart-owned drawing objects (`horizontal-line` and `trend-line`), while runtime selection/hit-test stays intentionally outside the persistence boundary; broader drawing tools, richer template versioning, and workspace persistence still remain outside the current boundary
