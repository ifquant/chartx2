...
- a first pane lifecycle surface now exists
  ...
  - `subscribePaneEvents() / unsubscribePaneEvents()` on the chart API
...
- pane handles now expose a small options surface, a resize callback, and a chart-level pane event bus, but pane-local APIs are still much narrower than lightweight-charts
