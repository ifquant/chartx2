...
<article class="demo-card workbench-card" data-demo-tab="workbench">
  <div class="card-head compact-head">
    ...
    <WorkbenchWorkspaceTabStrip
      tabs={workbench?.workspaceTabs ?? []}
      onSelectTab={(tabId) => handleSetWorkspaceTab(tabId)}
      onCloseTab={(tabId) => {
        closeMobileSheetsForNavigation();
        return onCloseWorkspaceTab(tabId);
      }}
      onCreateTab={() => {
        closeMobileSheetsForNavigation();
        return onCreateWorkspaceTab();
      }}
    />
  </div>
    ...
    >
      ...
      <div class="workbench-sidebar-scroll">
        ...
        <WorkbenchDrawingInspectorPanel
          ...
          {activeDrawingTool}
          pendingTrendLineStartTime={snapshot.drawingTool?.pendingTrendLineStartTime ?? null}
          {inspectorErrors}
