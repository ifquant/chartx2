...
import { createChartPublicSurfaceOwner } from "../../src/lib/chartx/internal/views/chart-public-surface-owner";
...
describe("chart public surface owner", () => {
  it("builds a public facade surface from stable owners", () => {
    const deps = createOwnerDeps();
    const surface = createChartPublicSurfaceOwner(deps as never).publicApiSurface();
  ...
  it("routes destructive public commands to their owners", () => {
    const deps = createOwnerDeps();
    const surface = createChartPublicSurfaceOwner(deps as never).publicApiSurface();

    surface.removeSeries({} as never);
    surface.clearSelectedDrawing();
    surface.removePaneByHandle("pane" as never);
    surface.clearTradeLocation();
    surface.detach();

    expect(deps.seriesCommandOwner.removeSeries).toHaveBeenCalledTimes(1);
    expect(deps.drawingOwner.clearSelectedDrawing).toHaveBeenCalledTimes(1);
    expect(deps.paneOwner.removePaneByHandle).toHaveBeenCalledWith("pane");
    expect(deps.tradeLocationOwner.clear).toHaveBeenCalledTimes(1);
    expect(deps.detach).toHaveBeenCalledTimes(1);
  });
});
